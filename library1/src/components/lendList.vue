    ...
    <div>
        <el-table :data="tableData" style="width: 100%">
            ...
            <el-table-column  label="操作" width="180">
                <template slot-scope="scope"><!--作用可以同各国scope绑定当前行得数据-->
                    <el-button v-on:click="deleteList(scope.row.b_id)" type="primary">还书</el-button>
                </template>
            </el-table-column>
        </el-table>
    </div>
...
<script>
    export default {
        ...
        data() {
            ...
            this.$http.get("http://localhost:8081/yding/findYD", {xhrFields: {withCredentials: true}}, {crossDomain: true}).then(req => {
